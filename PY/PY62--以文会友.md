...
```python
import os                               # 处理文件路径、目录操作
import zipfile                          # Python内置的压缩/解压模块
...
def zip_files(zip_path, files):
    ...
    with zipfile.ZipFile(zip_path, 'w') as zipf:
        for file in files:
            zipf.write(file)            # 逐个写入文件到压缩包

# 解压文件函数：把zip包全部解压到目标目录
def unzip_files(zip_path, extract_to):
    # 以只读模式打开压缩包
    with zipfile.ZipFile(zip_path, 'r') as zipf:
        zipf.extractall(extract_to)     # 一键解压所有文件

if __name__ == "__main__":
    # 1. 指定要压缩的文件夹路径
    directory_to_zip = 'E:\\Code\\PythonCode\\2024-6-22'
    # 2. 筛选出文件夹里所有文件（排除子文件夹），生成完整路径列表
    files_to_zip = [os.path.join(directory_to_zip, f)
                    for f in os.listdir(directory_to_zip)
                    if os.path.isfile(os.path.join(directory_to_zip, f))]
    # 3. 调用压缩函数，把文件打包成1.zip
    zip_files('1.zip', files_to_zip)
```

### 文本内容搜索与替换

!!! example 读取文件内容，搜索特定字符串并替换为新字符串。

```python
def search_and_replace(file_path, target, replacement):
    # 1. 以只读模式打开文件，读取全部内容
    with open(file_path, 'r', encoding='utf-8') as file:
        contents = file.read()

    # 2. 将内容中所有目标字符串替换为新字符串
    new_contents = contents.replace(target, replacement)

    # 3. 以写入模式打开文件，将替换后的内容写回（覆盖原文件）
    with open(file_path, 'w', encoding='utf-8') as file:
        file.write(new_contents)

if __name__ == "__main__":
    # 示例：把当前目录下1.txt文件里的"Hello"替换成"HI”
    search_and_replace(".\\1.txt", "Hello", "HI")
```
