...
<u>队列（queue）</u>是一种遵循先进先出规则的线性数据结构。顾名思义，队列模拟了排队现象，即新来的人不断加入队列尾部，而位于队列头部的人逐个离开。

如下图所示，我们将队列头部称为"队首"，尾部称为"队尾"，将把元素加入队尾的操作称为"入队"，删除队首元素的操作称为"出队"。

![队列的先进先出规则](DA3x.assets/queue_operations.png)

## 1. 队列常用操作

队列的常见操作如下表所示。需要注意的是，不同编程语言的方法名称可能会有所不同。我们在此采用与栈相同的方法命名。

<p align="center"> 表 1 &nbsp; 队列操作效率 </p>
...
# 访问队首元素 - 访问列表第一个元素，对应队列的peek操作
front: int = que[0]

# 元素出队 - 移除并返回列表第一个元素，对应队列的pop操作
pop: int = que.popleft()

# 获取队列的长度 - 获取列表长度，对应队列的size操作
size: int = len(que)

# 判断队列是否为空 - 检查列表长度是否为0，对应队列的isEmpty操作
...
if __name__ == "__main__":
    ...
    pop = que.popleft()
    print("出队元素 pop =", pop)
    print("出队后 que =", list(que))

    # 获取队列的长度
    size = len(que)
    print("队列的长度 size =", size)
...
if __name__ == "__main__":
    ...
    # 测试环形特性
    print("\n测试环形数组特性：")
    queue.push(6)  # 应该回到数组头部
    print("入队6后 queue =", queue.to_list())
```
...
- 基于链表的实现：入队和出队操作的时间复杂度都是 $O(1)$，但需要额外的内存分配和指针操作。
- 基于数组的实现：入队和出队操作的时间复杂度都是 $O(1)$，且具有更好的缓存局部性，效率较高。

**空间效率**

- 基于链表的实现：每个节点需要额外的指针空间，但可以动态增长，没有空间浪费。
- 基于数组的实现：可能存在空间浪费，特别是当队列长度远小于数组容量时。
...
## 3. The Queue Abstract Data Type

像栈一样，队列也是一种线性数据结构，它以先进先出（FIFO, First In First Out）的方式存储项目。在队列中，最早添加的项会最先被移除。队列的一个很好的<mark>例子是资源的消费者队列</mark>，其中最早来的消费者会被优先服务。这意味着，队列中的元素按照它们进入的顺序进行处理，确保了第一个进入队列的元素也是第一个从队列中出来的，体现了"先到先得"的原则。

> Like a stack, queue is a linear data structure that stores items in a First In First Out (FIFO) manner. With a queue, the least recently added item is removed first. A good example of a queue is any queue of consumers for a resource where the consumer that came first is served first.
...
The queue abstract data type is defined by the following structure and operations. A queue is structured, as described above, as <mark>an ordered collection of items which are added at one end, called the "rear," and removed from the other end, called the "front."</mark> Queues maintain a FIFO ordering property. The queue operations are given below.
...
## 4. Implementing a Queue in Python
...
class Queue:
    ...
    def enqueue(self, item):
        """入队操作 - 在位置0插入元素（O(n)复杂度）"""
        self.items.insert(0, item)

    def dequeue(self):
        """出队操作 - 移除并返回末尾元素（O(1)复杂度）"""
...
def hot_potato(name_list, num):
    ...
    while len(queue) > 1:
        # 将前num个元素依次取出并放到列表末尾，相当于循环传递
        for i in range(num):
            queue.append(queue.pop(0))	# O(N) - 从列表头部取出并放到尾部
        # 移除第num+1个元素（即数到m的猴子）
        queue.pop(0)						# O(N)
...
def hot_potato(name_list, num):
    ...
    while len(queue) > 1:
        # 将前num个元素依次取出并放到列表末尾，相当于循环传递
        for i in range(num):
            queue.append(queue.popleft()) # O(1) - 从队列头部取出并放到尾部
        # 移除第num+1个元素（即数到m的猴子）
        queue.popleft()
...
❏ 如果打印任务执行完毕，或者说任务需要的时间减为0，则说明打印机回到空闲状态None。
...
class Printer:
    ...
    def __init__(self, ppm):
        """构造方法 - ppm表示每分钟打印页数"""
        self.page_rate = ppm  # 打印速度（页/分钟）
        self.current_task = None  # 当前正在打印的任务
...
def simulation(num_seconds, pages_per_minute):
    """模拟打印机运行"""
    lab_printer = Printer(pages_per_minute)  # 创建打印机
    print_queue = Queue()  # 创建打印任务队列
    waiting_times = []  # 存储等待时间
...
# 测试代码 - 模拟10次，每次3600秒（1小时），每分钟打印10页
for i in range(10):
...
- `addFront(item)` adds a new item to front of the deque. It needs the item and returns nothing.
- `addRear(item)` adds a new item to rear of the deque. It needs the item and returns nothing.
- `removeFront()` removes the front item from the deque. It needs no parameters and returns the item. The deque is modified.
- `removeRear()` removes the rear item from the deque. It needs no parameters and returns the item. The deque is modified.
- `isEmpty()` tests to see whether the deque is empty. It needs no parameters and returns a boolean value.
- `size()` returns number of items in the deque. It needs no parameters and returns an integer.
...
if __name__ == "__main__":
    ...
    print("队首出队后 deque =", list(deq))
    pop_rear = deq.pop()  # 队尾元素出队
    print("队尾出队元素  pop_rear =", pop_rear)
    print("队尾出队后 deque =", list(deq))

    # 获取双端队列的长度
    size = len(deq)
    print("双端队列长度 size =", size)

    # 判断双端队列是否为空
    is_empty = len(deq) == 0
    print("双端队列是否为空 =", is_empty)
```
...
if __name__ == "__main__":
    ...
    deque.push_last(5)
    deque.push_last(4)
    print("队尾入队后:", deque.to_array())
    
    # 队首入队
    deque.push_first(3)
    deque.push_first(1)
    print("队首入队后:", deque.to_array())
    
    # 访问元素
    print("队首元素:", deque.peek_first())
    print("队尾元素:", deque.peek_last())
    
    # 队首出队
    print("队首出队:", deque.pop_first())
    print("队首出队后:", deque.to_array())
    
    # 队尾出队
    print("队尾出队:", deque.pop_last())
    print("队尾出队后:", deque.to_array())
```
...
### 7.3.2 示例04067: 回文数字（Palindrome Number）
...
- 队列是一种遵循先进先出原则的数据结构，同样可以通过数组或链表来实现。在时间效率和空间效率的对比上，队列的结论与前述栈的结论相似。
