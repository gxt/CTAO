...
![第一次 DFS 的深度优先森林与发现/结束时间](assets/07--sccforest.png)

**第二步：构建转置图**
所有边的方向反向：0←1, 1←2, 2←3, 2←4, 4←3（已在图中标出）

![转置图](assets/07--transpose1.png)
![转置图上的第二次 DFS](assets/07--transpose2.png)
...
https://sunnywhy.com/sfbj/10/8/407

标准 SCC 检测，输入 n 个顶点 m 条边的有向图，输出 SCC 的个数。
与 Kosaraju 配套使用：第一次 DFS 求结束顺序，第二次 DFS 在转置图上按逆序找 SCC。
...
https://sunnywhy.com/sfbj/10/8/408

输出每个顶点的 SCC 编号，要求编号按拓扑序（缩点后 DAG 的顺序）排列。
Tarjan 一次 DFS 即可完成，适合理解 lowlink 的工作原理。
