...
![Prim 步骤 1：从 A 开始](assets/06--prima.png)
![Prim 步骤 2：选 A-C](assets/06--primb.png)
![Prim 步骤 3：选 C-B](assets/06--primc.png)
![Prim 步骤 4](assets/06--primd.png)
![Prim 步骤 5](assets/06--prime.png)
![Prim 步骤 6：选 B-D](assets/06--primf.png)
![Prim 步骤 7：完成](assets/06--primg.png)
...
### 3.4 附：Prim 与 Dijkstra 的代码对比

Prim 算法和 Dijkstra 算法的代码结构惊人地相似——这并非偶然，因为它们共享同一个**贪心框架**：

1. 维护一个优先队列，每次取"当前最优"的顶点
2. 用该顶点更新其邻居的距离
3. 重复直至所有顶点处理完毕
...
Dijkstra 更新逻辑：
```python
nd = d + u.getWeight(v)   # 累加路径长度：起点 → u → v
```

Prim 更新逻辑：
```python
w = u.getWeight(v)        # 只取边权：从当前树到 v 的最小边
```

下面将两个算法的完整代码并排放置，关键差异用 `***` 标出：
...
def dijkstra_oop(graph, start):
    ...
    while pq:
        ...
        visited.add(u)
        for v in u.getConnections():
            nd = d + u.getWeight(v)    # *** Dijkstra: 累加路径
            if nd < v.distance:
                v.distance = nd
                v.pred = u
...
| | Dijkstra | Prim |
|---|---|---|
| 更新值 | `d + weight`（累加路径长度） | `weight`（仅边权） |
| 目标 | 求起点到各点的最短路径 | 求连接所有顶点的最小总边权 |
| 适用图 | 有向/无向（非负权） | 无向 |

**直观理解**：
- Dijkstra 关心"从起点出发到这里要走多远"（**路径长度**）
- Prim 关心"把当前顶点连到树上要花多少"（**扩展代价**）

两段代码完全相同，唯有一行不同——这正是算法教学中值得品味的经典对比。
...
最小生成树，https://leetcode.cn/problems/min-cost-to-connect-all-points/description/
...
新增测试用例：更多点
```
points = [[0,0],[1,1],[2,0],[3,3],[0,3],[1,2]]
```
