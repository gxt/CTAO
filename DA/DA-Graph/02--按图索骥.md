...
![词梯通配符桶建图](assets/02--wordbuckets.png)
![词梯图](assets/02--wordgraph.png)
...
![骑士的 8 种走法](assets/02--knightmoves.png)
...
def gen_legal_moves(row, col, board_size):
    """
    生成棋盘上 (row, col) 位置骑士的所有合法走法
    
    参数:
        row, col: 当前格子的行/列坐标（从 0 开始）
        board_size: 棋盘边长
    ...
    for dr, dc in moves:
        ...
        if 0 <= nr < board_size and 0 <= nc < board_size:
            result.append(nr * board_size + nc)
    return result
...
def build_knight_graph(board_size):
    ...
    for row in range(board_size):
        for col in range(board_size):
            ...
            graph[node_id] = gen_legal_moves(row, col, board_size)
    return graph
```
...
![8×8 棋盘骑士周游完整路径](assets/02--bigknight.png)
![Warnsdorff 启发式：优先选出路少的顶点](assets/02--moveCount.png)

### 3.4 图构建模式归纳

回顾本章的两种建图方式——词梯图和骑士周游图，它们遵循同一个通用范式：

| 步骤 | 词梯图 | 骑士周游图 |
|---|---|---|
| **确定顶点** | 词典中的单词 | 棋盘上的每格（线性编号） |
| **确定边规则** | 只差一个字母的单词相连 | 合法"马走日"移动 |
| **构建策略** | 通配符桶避免 $O(n^2)$ | 8 种偏移量 + 边界检查 |
| **求解算法** | BFS（求最短路径） | DFS（搜索哈密顿路径） |

共同思维：**将现实问题转化为图，顶点和边的定义方式决定了后续可用哪种图算法求解**。
