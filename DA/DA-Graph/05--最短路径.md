...
![Dijkstra 步骤 1：初始化](assets/05--dijkstraa.png)
![Dijkstra 步骤 2](assets/05--dijkstrab.png)
![Dijkstra 步骤 3](assets/05--dijkstrac.png)
![Dijkstra 步骤 4](assets/05--dijkstrad.png)
![Dijkstra 步骤 5](assets/05--dijkstrae.png)
![Dijkstra 步骤 6：完成](assets/05--dijkstraf.png)
...
新增测试用例：更多节点
```
times = [[1,2,2],[1,3,4],[2,4,7],[2,5,3],[3,4,1],[3,5,5],[4,6,2],[5,6,1]]
n = 6, k = 1
```
...
Bellman-Ford，https://leetcode.cn/problems/cheapest-flights-within-k-stops/description/

有 n 个城市，m 条航线。每条航线从 u 到 v，价格为 w。求从 src 到 dst 最多经过 k 次中转的最便宜价格。

**思路**：Bellman-Ford 的变体，限制松弛轮数不超过 k+1。
...
### 6.8 课堂练习：修复 Dijkstra 的 Bug

以下 Dijkstra 实现有一个常见的错误。你能找到并修复它吗？

```python
def dijkstra_buggy(graph, start):
    n = len(graph)
    dist = [float('inf')] * n
    dist[start] = 0
    visited = [False] * n
...
**问题分析**：`pq.pop(0)` 是列表的弹出操作，时间复杂度 O(n) 且弹出的是**第一个元素**而非最小元素。当堆中有多个距离不同的顶点时，无法保证每次取出的是距离最小的顶点。

**修复方法**：将 `pq.pop(0)` 替换为 `heapq.heappop(pq)`，并将 `pq` 初始化为堆：
...
def dijkstra_fixed(graph, start):
    ...
    while pq:
        d, u = heapq.heappop(pq)     # 正确：弹出最小元素
        if d != dist[u]:             # 等价于 visited 检查
            continue
        for v, w in graph[u]:
            if d + w < dist[v]:
                dist[v] = d + w
